...
interface Holder: 
    #def setAvailable(name: Srting[35]): nonpayable
    def savePost(post: Post): nonpayable
    def viewPost(link: String[100]) -> Post: view
    
enum PostStatus:
    AVAILABLE #el sprite existe para descargar
    UNAVAILABLE #no existe ni una wea
...
def postSprite(name: String[35], receiverAddress: address, dlLink: String[100], pr: uint256):
    ...
    post: Post = Post({sprite: spriteToPost, sender: self.owner, receiver: receiverAddress, downloadLink: dlLink, uploadDate: block.timestamp, price: pr, status: PostStatus.AVAILABLE})
    holder: Holder = Holder(receiverAddress)
    holder.savePost(post)
    
    
@external
def createSprite(newName: String[35], newLinkIcon: String[50], pixelW: uint256, pixelH: uint256, size: uint256, pr: uint256):
